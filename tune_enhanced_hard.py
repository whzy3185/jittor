...
def _test_candidate_sources(test_path: Path) -> tuple[np.ndarray, dict[int, np.ndarray]]:
    ...
    return unique, {k: np.unique(np.asarray(v, dtype=np.int64)) for k, v in by_src.items()}
...
def feature_frame(candidates: pd.DataFrame, stats) -> np.ndarray:
    out = np.zeros((len(candidates), len(ENHANCED_COMPONENT_NAMES)), dtype=np.float64)
    for _, idx in candidates.groupby("query_id", sort=False).groups.items():
        ...
        out[idx] = enhanced_component_matrix(int(g["src"].iloc[0]), float(g["time"].iloc[0]), g["dst"].to_numpy(np.int64), stats)
    return out
...
def score_frame(candidates: pd.DataFrame, stats, weights: np.ndarray) -> np.ndarray:
    ...
    for _, idx in candidates.groupby("query_id", sort=False).groups.items():
        idx = np.asarray(list(idx), dtype=np.int64)
        g = candidates.iloc[idx]
        raw = score_enhanced_candidates(int(g["src"].iloc[0]), float(g["time"].iloc[0]), g["dst"].to_numpy(np.int64), stats, weights)
        out[idx] = _rank01(raw)
    return out
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/enhanced_heuristic_hard")
    ...
    print(json.dumps({"dataset": args.dataset, "history_edges": len(hist), "val_edges": len(val), "candidates": len(candidates), "cooc_pairs": len(stats.cooc)}, ensure_ascii=False))
    ...
    best_w = ENHANCED_DEFAULT_WEIGHTS.copy()
    ...
    for trial in range(args.trials):
        if trial < args.trials // 2:
            w = best_w * rng.lognormal(0.0, 0.22, size=len(best_w)) + rng.normal(0.0, 0.06, size=len(best_w))
        else:
            w = ENHANCED_DEFAULT_WEIGHTS * rng.lognormal(0.0, 0.65, size=len(best_w)) + rng.normal(0.0, 0.22, size=len(best_w))
    ...
    save_json(
        output_dir / f"{args.dataset}_enhanced_heuristic.json",
        {
            "dataset": args.dataset,
            "component_names": ENHANCED_COMPONENT_NAMES,
            "weights": best_w.astype(float).tolist(),
            "default_weights": ENHANCED_DEFAULT_WEIGHTS.astype(float).tolist(),
            "best_metrics": best_metrics,
            "args": vars(args),
        },
    )
    pd.DataFrame(rows).to_csv(output_dir / f"{args.dataset}_enhanced_tuning_log.csv", index=False)
    print(json.dumps({"best": best_metrics, "weights": dict(zip(ENHANCED_COMPONENT_NAMES, best_w.astype(float).tolist()))}, ensure_ascii=False, indent=2))
