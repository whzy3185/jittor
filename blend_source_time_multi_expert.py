...
def _thresholds(test_csv: Path, chunksize: int, src_q: float, time_q: float) -> tuple[Counter[int], float, float]:
    ...
    time_threshold = float(np.quantile(times, time_q))
...
def _blend_dataset2(
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as fout:
        ...
        top2_iter = pd.read_csv(top2_csv, header=None, chunksize=chunksize)
        top4_iter = pd.read_csv(top4_csv, header=None, chunksize=chunksize)
        top5_iter = pd.read_csv(top5_csv, header=None, chunksize=chunksize)
        test_iter = pd.read_csv(test_csv, usecols=["src", "time"], chunksize=chunksize)
        for base_chunk, top2_chunk, top4_chunk, top5_chunk, test_chunk in zip(
            base_iter, top2_iter, top4_iter, top5_iter, test_iter
        ):
            base = base_chunk.to_numpy(np.float64)
            top2 = top2_chunk.to_numpy(np.float64)
            top4 = top4_chunk.to_numpy(np.float64)
            top5 = top5_chunk.to_numpy(np.float64)
            srcs = test_chunk["src"].to_numpy(np.int64)
            times = test_chunk["time"].to_numpy(np.float64)
            out = np.empty_like(base)
            for i, (src, t) in enumerate(zip(srcs, times)):
                active = float(counts[int(src)]) >= src_threshold
                late = float(t) >= time_threshold
                if active and late:
                    expert = top5[i]
                    weight = active_late_weight
                    active_late_rows += 1
                elif active:
                    expert = top4[i]
                    ...
                    weight = low_weight
                    low_rows += 1
                out[i] = (1.0 - weight) * base[i] + weight * expert
...
def main() -> None:
    ...
    parser.add_argument("--top2-dir", required=True)
    parser.add_argument("--top4-dir", required=True)
    parser.add_argument("--top5-dir", required=True)
    ...
    parser.add_argument("--active-late-weight", type=float, default=0.64)
    ...
    rows2, stats = _blend_dataset2(
        ...
        Path(args.top2_dir) / "dataset2.csv",
        Path(args.top4_dir) / "dataset2.csv",
        Path(args.top5_dir) / "dataset2.csv",
    ...
    print(
        {
            ...
            "weights": {
                "low_top2": args.low_weight,
                "active_top4": args.active_weight,
                "late_top4": args.late_weight,
                "active_late_top5": args.active_late_weight,
            },
            "stats": stats,
        }
    )
