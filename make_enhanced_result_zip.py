...
from enhanced_heuristic import ENHANCED_DEFAULT_WEIGHTS, build_enhanced_stats, score_enhanced_candidates
...
def _write_dataset(
    ...
    model_path = model_dir / f"{dataset}_enhanced_heuristic.json"
    weights = np.asarray(load_json(model_path)["weights"], dtype=np.float64) if model_path.exists() else ENHANCED_DEFAULT_WEIGHTS
    print({"dataset": dataset, "history_edges": len(train), "candidate_pool": len(pool), "cooc_pairs": len(stats.cooc)}, flush=True)
    ...
    with output_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for chunk in pd.read_csv(test_path, chunksize=chunk_size):
            ...
            for i in range(len(chunk)):
                raw = score_enhanced_candidates(int(srcs[i]), float(times[i]), cmat[i], stats, weights)
...
def main():
    ...
    parser.add_argument("--model-dir", type=str, default="outputs/enhanced_heuristic_hard")
    parser.add_argument("--output-dir", type=str, default="outputs/website_submission_enhanced_hard_rank")
